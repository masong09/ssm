	...
	<div class="wrapper">
		...
		<div class="content-wrapper">
			...
			<ol class="breadcrumb">
				...
				<li><a href="${pageContext.request.contextPath}/user/queryUserList.action">用户管理</a></li>
						...
						<div class="table-box">
							...
							<div class="pull-left">
								<div class="form-group form-inline">
									<div class="btn-group">
										<button type="button" class="btn btn-default" title="新建" onclick="location.href='${pageContext.request.contextPath}/user/toUserAdd.action'">
								...
								<tbody>
									<c:forEach items="${pageInfo.list}" var="user" varStatus="status">
										<tr>
											...
											<td>${user.email}</td>
											<td>${user.phoneNum}</td>
											<td>${user.status==1?'未锁定':'已锁定'}</td>
											<td class="text-center">
												<a href="${pageContext.request.contextPath}/user/queryUserById.action?id=${user.id}" class="btn bg-olive btn-xs">详情</a>
												<a href="${pageContext.request.contextPath}/user/toUserRoleAdd.action?id=${user.id}" class="btn bg-olive btn-xs">添加角色</a>
											</td>
										</tr>
									</c:forEach>
								</tbody>
						...
						<div class="box-tools pull-right">
							<ul class="pagination">
								...
								<c:forEach begin="1" end="${pageInfo.pages}" var="page">
									<li class="${page==pageInfo.pageNum?'active':''}"><a href="javascript:gotoPage('${page}')">${page}</a></li>
								</c:forEach>
		...
		<script>
                    ...
                    $("#pageSizeSel option[value='${pageInfo.pageSize}']").prop("selected",true);
                    ...
                    function  gotoPage(pageNum) {
                        ...
                        if(pageNum >=1 && pageNum <=${pageInfo.pages}) {
                            ...
                            location.href = "${pageContext.request.contextPath}/user/queryUserList.action?pageNum="+pageNum+"&pageSize="+pageSize;
                        }else{
                            //页码不合法，默认跳转第一页
                            gotoPage("1");
                        }
