...
<body class="hold-transition skin-blue sidebar-mini">
			...
			<div class="box box-primary">
				...
				<div class="box-body">
					...
					<div class="table-box">
							...
							<div class="form-group form-inline">
								<div class="btn-group">
									<button type="button" class="btn btn-default" title="新建"
										onclick='location.href="${pageContext.request.contextPath}/orders/toOrderAdd.action"'>
						...
						<form id="dataForm" action="${pageContext.request.contextPath}/orders/deleteOrders.action">
							<!--数据列表-->
							<table id="dataList"
								...
								<c:forEach items="${pageInfo.list}" var="orders" varStatus="status">
									<tr>
										...
										<td>${status.count}</td>	<%--每页的序号都从1开始--%>
										<td>${orders.orderNum}</td>
										<td>${orders.orderTime}</td>
										<td>${orders.peopleCount}</td>
										<td>${orders.orderDesc}</td>
										<td>${orders.payType==0?'支付宝':orders.payType==1?'微信':'其他'}</td>
										<td>${orders.orderStatus==0?'未支付':'已支付'}</td>
										...
										<td class="text-center">
											...
											<button type="button" class="btn bg-olive btn-xs"
												onclick='location.href="${pageContext.request.contextPath}/orders/toUpdateOrders.action?id=${orders.id}"'>查看</button>
										</td>
									</tr>
								</c:forEach>
								</tbody>
	...
	<script>
		...
		function gotoPage(pageNum) {
			...
			location.href = "${pageContext.request.contextPath}/orders/queryOrdersList.action?pageNum="+pageNum+"&pageSize="+pageSize;
		    }else{
                        //页码不合法，默认跳转第一页
                        gotoPage("1");
		    }
